...
function useInit(ChartClass, config) {
    ...
    var reactDomToString = function (source, path, type) {
        var statisticCustomHtml = util_2.hasPath(source, path);
        util_2.setPath(source, path, function () {
            var arg = [];
            for (var _i = 0; _i < arguments.length; _i++) {
                arg[_i] = arguments[_i];
            }
            var statisticDom = util_2.isType(statisticCustomHtml, 'Function')
                ? statisticCustomHtml.apply(void 0, arg) : statisticCustomHtml;
            if (util_2.isType(statisticDom, 'String') ||
                util_2.isType(statisticDom, 'Number') ||
                util_2.isType(statisticDom, 'HTMLDivElement')) {
                return statisticDom;
            }
    ...
    var processConfig = function () {
        // statistic
        if (util_2.hasPath(config, ['statistic', 'content', 'customHtml'])) {
            reactDomToString(config, ['statistic', 'content', 'customHtml']);
        }
        if (util_2.hasPath(config, ['statistic', 'title', 'customHtml'])) {
            reactDomToString(config, ['statistic', 'title', 'customHtml']);
        }
        // tooltip
        if (typeof config.tooltip === 'object') {
            if (util_2.hasPath(config, ['tooltip', 'container'])) {
                reactDomToString(config, ['tooltip', 'container'], 'tooltip');
            }
            if (util_2.hasPath(config, ['tooltip', 'customContent'])) {
    ...
    react_1.useEffect(function () {
        if (chart.current && !util_1.isEqual(chartOptions.current, config)) {
            ...
            chartOptions.current = util_2.deepClone(config);
        }
    }, [config]);
    react_1.useEffect(function () {
        ...
        chart.current = util_2.clone(chartInstance);
        if (onReady) {
