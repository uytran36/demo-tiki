...
const getAmountLSBD_NVGH = async () => {
  try {
    let pool = await sql.connect(config);
    const getAmount = await pool.request().execute("getAmountLSBD_NVGH");
...
const getAmountLSBD_NVQLK = async () => {
  try {
    let pool = await sql.connect(config);
    const getAmount = await pool.request().execute("getAmountLSBD_NVQLK");
...
const getNVQLK = async () => {
  try {
    let pool = await sql.connect(config);
    const getNVQLK = await pool.request().execute("getNVQLK");
...
const updateAuthNVQLK = async (MaNV, NVData) => {
  try {
    ...
    const up = await pool
      ...
      .input("MaKho", sql.Int, NVData.MaKho)
      .input("MaNV", sql.Int, MaNV)
      .execute("NVQLKmoi");
...
const getKho = async () => {
  try {
    let pool = await sql.connect(config);
    const getKho = await pool.request().execute("getKho");
